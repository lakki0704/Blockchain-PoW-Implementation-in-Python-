...
class Blockchain:
    ...
    def _to_digest(self, new_proof: int, previous_proof: int, index: int, data: str) -> bytes:
        to_digest = str(new_proof ** 2 - previous_proof ** 2 + index) + data #a mathematical function that determines the proof_hash
        # It returns an utf-8 encoded version of the string
        return to_digest.encode()

    def _proof_of_work(self, previous_proof: int, index: int, data: str) -> int:
        #it starts with some number and stops at a number that meets any particular requirement / mining mathematical function ig
        new_proof = 1
        check_proof = False
        while not check_proof:
            to_digest = self._to_digest(new_proof, previous_proof, index, data)
            hash_operation = _hashlib.sha256(to_digest).hexdigest()
            if hash_operation[:4] == "0000": #more zeroes , more computational energy
                check_proof = True
            else:
                new_proof += 1
        return new_proof
